...
| `test_create_notifies_when_threshold_crossed` | U3.1 notification-flow invariant: `fill_pct >= notify_threshold` → `notification_service.send(user, category, fill_pct)` called exactly once (single-member account) with the right args |
...
| `test_create_notifies_all_account_members` | U1.4/D104: 2 members in `user_repo` → `notification_service.send()` called once per member |
| `test_create_notify_failure_for_one_member_does_not_block_others_or_expense` | One recipient's `send()` raising doesn't stop the fan-out loop (the other member still gets notified) or the expense creation |
| `test_create_notifies_single_member_account_once_no_dupes` | A single-member account still notifies exactly once — no duplicate sends |
...
| `test_create_expense_triggers_notification_when_threshold_crossed` | End-to-end U3.1/U1.4 wiring through the real `get_expense_service` factory: crossing the threshold on `POST /expenses` calls the (faked) `notification_service.send()` once per account member (fan-out, D104) |
...
| `test_get_me_includes_account_currency_and_name` | `GET /users/me` response includes `currency` and `account_name` matching the caller's account (D211, U0.2c) — same `accounts` join, no second query |
| `test_list_users_response_has_no_currency_field` | `GET /users` (every route but `/me`) has no `currency`/`account_name` field — `UserResponse` is untouched by D211/U0.2c |
...
## E2e smoke (`test_e2e_smoke.py`) — MVP U5.1 + family-features-v1_1 U3.1 + mini-app-v2 U3.1 + mini-app-v3 U4.1 + mini-app-v4 U4.1 + mini-app-v5 U4.1, `@pytest.mark.integration`, excluded from default `verify.sh`
...
| `test_ramp_slot_and_defaulted_notify_threshold_through_init_data` | A category created with `color_slot: 72` (the widened palette's top slot, D500) reads back 72 in both the create response and `GET /categories`; `color_slot: 73` is 422 and writes nothing, proven by re-listing and finding only the accepted category; a budget created with no `notify_threshold` reads back the new default 70 (D507); a second budget, in a different category (the `UNIQUE (category_id, account_id, period)` constraint forbids a second plan on the same one), with an explicit `85` reads back 85 unchanged (mini-app-v5 U4.1 AC) |
