...
- Admin surfaces beyond the existing panel: no in-app way to grant
  `system_admin` itself (still direct DB access only), no bulk user
  management. The panel that exists (`screens/admin.ts`, U4.7–U4.10) covers
  account/user block-unblock and creating a new account plus its first user.
